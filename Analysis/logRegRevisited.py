...
def main():
	...
	x.drop(labels=columnList,inplace=True, axis=1)
	x.drop(list(x.filter(regex = 'ANCESTRY')), axis = 1, inplace = True)
	x.drop(list(x.filter(regex = 'LANGUAGE')), axis = 1, inplace = True)
	y = merged.iloc[:,-1:]
